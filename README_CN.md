...
- **按角色分配模型。** 强模型负责设计和审查（推理密集型）；高效模型负责实现（执行密集型）。可在 agent 文件中配置。
...
每个 agent 的 frontmatter 有 `model:` 字段。编辑文件替换为你的实际模型名：
...
- agent frontmatter 的 `model:` 字段是对编排者的建议。实际模型强制取决于 Codex 的委派实现。
